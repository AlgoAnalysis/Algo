...
public class RunBollingerReversalStrategyCheck {
	...
	public RunBollingerReversalStrategyCheck(){}
	...
	public static void main(String [] args)
	  {
		...
		for (int i = 0; i < listOfFiles.length; i++) {
			if (listOfFiles[i].isFile() && listOfFiles[i].getName().endsWith(".csv")) {
				...
				new RunBollingerReversalStrategyCheck().getPinbarStatisticsForFile(comodityName, listOfFiles[i].getName(), appendTotalResults);
			} 
		}
	  }
...
//	    Map<Integer, CandleDaySuccessStatistics> daystatistics = new HashMap<Integer, CandleDaySuccessStatistics>();
	    ...
	    try
	    {
	    	...
	    	try
	    	{
	    		String calculationResults = "C:\\Users\\ohad\\AppData\\Roaming\\MetaQuotes\\Terminal\\1CF1A45F9E06881833077514E2850EFB\\MQL4\\Files\\test_results_for_tal\\" + comodityName + "_bollinger_reversal_" + timeFrame + ".csv";
	    		...
	    		Strategy barStrategy = new BollingerBandsReversal();
	    		...
	    		for (SingleCandleBarData temp : datalist) {
	    			...
	    			if (barStrategy.isStrategyPattern(candleBars, index, pipsValue)) {
	    				...
	    				currentBarWriter.append(',');
	    				currentBarWriter.append("" + ((BollingerBandsReversal)barStrategy).getRisk());
	    				currentBarWriter.append(',');
	    				double pinbarGain = (double)Math.round( (bullishPinbarStrategy ? barStrategy.getLongGain(candleBars, index + 1, ((BollingerBandsReversal)barStrategy).getStopLoss(), ((BollingerBandsReversal)barStrategy).getConfirmationPoint() ) : barStrategy.getShortGain(candleBars, index + 1, ((BollingerBandsReversal)barStrategy).getStopLoss(), ((BollingerBandsReversal)barStrategy).getConfirmationPoint()) ) * (double) 1000) / (double)1000;
	    				...
	    				currentBarWriter.append(',');
	    				double riskGainRatio = (double)Math.round( (bullishPinbarStrategy ? barStrategy.getLongRiskGainRatio(candleBars, index, ((BollingerBandsReversal)barStrategy).getStopLoss(), ((BollingerBandsReversal)barStrategy).getConfirmationPoint()) : barStrategy.getShortRiskGainRatio(candleBars, index, ((BollingerBandsReversal)barStrategy).getStopLoss(), ((BollingerBandsReversal)barStrategy).getConfirmationPoint())) * (double) 1000) / (double)1000;
